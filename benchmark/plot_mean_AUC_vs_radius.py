...
main_df['radius'] = new[1].replace('R.: ', '', regex=True)
main_df['radius'] = main_df['radius'].astype(int) 
...
for index, conf in enumerate(configurations_per_plot):
    ...
    for g_index, group in sel_df.groupby('cluster'):
        # print(group)
        group.plot(y = "max_mean", x = "radius", yerr="max_err", ax = ax, label=g_index, capsize=4)

        # print(group) 

    # sel_df.plot(y = "max_mean", x = "radius", ax = ax)
    ...
    fig.tight_layout()
    plt.savefig('auc_vs_radius_line_plot_{0}_{1}.pdf'.format(conf[0].replace(' ','_'), conf[1].replace(' ','_')))
    # plt.show()
    # exit()
